...
class Linear_ReLU(object):
    ...
    def forward(x, w, b):
        ...
        out, z = ReLU.forward(Linear.forward(x, w, b)[0])
        cache = (x, w, b, z)
    ...
    def backward(dout, cache):
        ...
        dx, dw, db = Linear.backward(ReLU.backward(dout, cache[3]), cache[:3])
...
def softmax_loss(x, y):
    ...
    loss = torch.sum(torch.log(torch.sum(torch.exp(x), 1)) - x[range(len(y)), y]) / x.shape[0]
    dx = ((torch.exp(x).T / torch.sum(torch.exp(x), 1)).T - torch.nn.functional.one_hot(y, x.shape[1]).to('cpu' if (-x.get_device()) else 'cuda')) / x.shape[0]
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        self.params = {'W1': torch.normal(0, weight_scale, size=(input_dim, hidden_dim), dtype=dtype).to(device),
                       'b1': torch.zeros(hidden_dim, dtype=dtype).to(device),
                       'W2': torch.normal(0, weight_scale, size=(hidden_dim, num_classes), dtype=dtype).to(device),
                       'b2': torch.zeros(num_classes, dtype=dtype).to(device)}
    ...
    def loss(self, X, y=None):
        ...
        scores, cache2 = Linear.forward(y1, self.params['W2'], self.params['b2'])
        ...
        loss += self.reg * (torch.sum(torch.mul(self.params['W1'], self.params['W1'])) + torch.sum(
            torch.mul(self.params['W2'], self.params['W2'])))
        dx, dw, db = Linear.backward(dx, cache2)
        grads['b2'] = db
        grads['W2'] = dw + self.reg * 2 * self.params['W2']
        dx, dw, db = Linear_ReLU.backward(dx, cache1)
        grads['b1'] = db
        grads['W1'] = dw + self.reg * 2 * self.params['W1']
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers-1):
            self.params['W' + str(i)] = torch.normal(0, weight_scale, size=(n, hidden_dims[i]), dtype=dtype).to(device)
            self.params['b' + str(i)] = torch.zeros(hidden_dims[i], dtype=dtype).to(device)
            n = hidden_dims[i]

        self.params['W' + str(self.num_layers - 1)] = torch.normal(0, weight_scale, size=(n, num_classes), dtype=dtype).to(device)
        self.params['b' + str(self.num_layers-1)] = torch.zeros(num_classes, dtype=dtype).to(device)
    ...
    def loss(self, X, y=None):
        ...
        x = X
        for i in range(self.num_layers - 1):
            x, cache = Linear_ReLU.forward(x, self.params['W'+str(i)], self.params['b'+str(i)])
            caches['LR'+str(i)] = cache
            if self.use_dropout:
                x, cache = Dropout.forward(x, self.dropout_param)
                caches['D'+str(i)] = cache

        scores, cache = Linear.forward(x, self.params['W'+str(self.num_layers - 1)], self.params['b'+str(self.num_layers - 1)])
        caches['L'] = cache
        ...
        loss, dx = softmax_loss(scores, y)
        loss += self.reg * torch.sum(torch.mul(self.params['W' + str(self.num_layers-1)], self.params['W' + str(self.num_layers-1)]))
        dx, dw, db = Linear.backward(dx, caches['L'])
        grads['b' + str(self.num_layers-1)] = db
        grads['W' + str(self.num_layers-1)] = dw + self.reg * 2 * self.params['W' + str(self.num_layers-1)]

        for i in range(self.num_layers-2, -1, -1):
            if self.use_dropout:
                dx = Dropout.backward(dx, caches['D'+str(i)])

            loss += self.reg * torch.sum(torch.mul(self.params['W'+str(i)], self.params['W'+str(i)]))
            dx, dw, db = Linear_ReLU.backward(dx, caches['LR'+str(i)])
            grads['b' + str(i)] = db
            grads['W' + str(i)] = dw + self.reg * 2 * self.params['W' + str(i)]
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = Solver(model, data_dict, optim_config={'learning_rate': 1e-1},
                    lr_decay=0.95, num_epochs=20, batch_size=32, print_every=1000, device=device)
...
def adam(w, dw, config=None):
    ...
    next_w = w - config['learning_rate'] * (config['m'] / (1 - config['beta1'] ** config['t'])) / (torch.sqrt(config['v'] / (1 - config['beta2'] ** config['t'])) + config['epsilon'])
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            mask = (torch.rand(x.shape, device=('cpu' if (-x.get_device()) else 'cuda')) > p) / (1 - p)
            out = x * mask
